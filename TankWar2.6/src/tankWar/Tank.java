...
public class Tank {
	...
	private static final int MLIFE = 100;
	...
	public void keyPressed(KeyEvent e){
		...
		switch(keyValue){
		case KeyEvent.VK_F2 ://F2怎么都不行，总是在调节音量、需要按住Fn加F2才是F2最原来的功能
			if(!this.live){
				this.live = true;
				this.setLife(MLIFE);
			}
	...
	public boolean eat(Blood b){
		if(this.isLive() && b.getLive() && this.getRect().intersects(b.getRect())){
			this.setLife(MLIFE);
			b.setLive(false);
			return true;
		}
	...
	private class BloodBar{
		public void draw(Graphics g){
			...
			g.fillRect(x, y - 10, WIDTH * life / MLIFE, 10);
			g.setColor(c);
		}
	}
}
